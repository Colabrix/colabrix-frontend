...
const generateData = () => {
  const columnTitles = ['BACKLOG', 'TO DO', 'IN PROGRESS', 'BLOCKED', 'REVIEW', 'QA TESTING', 'UAT', 'DEPLOYMENT', 'DONE', 'ARCHIVED'];
  const columns = {};
  let idCounter = 1;
...
const DropIndicator = ({ isVisible }) => {
  return (
    <motion.div
      ...
      animate={isVisible ? { height: 4, opacity: 1, marginBottom: 8 } : { height: 0, opacity: 0, margin: 0 }}
    ...
    <>
      ...
      >
        <motion.div
          ...
          className="bg-white p-3 rounded-xl border border-gray-200 shadow-sm hover:shadow-md cursor-grab active:cursor-grabbing mb-3 transition-shadow duration-200 select-none"
        >
          {/* Decorative Left Border */}
          <div className={cn(
            "absolute -left-[1px] top-[14px] w-1 h-[60%] rounded-r-full",
          ...
          <div className="flex gap-2 mb-3 flex-wrap pl-2">
            {item.tags.map((tag, idx) => (
              <span key={idx} className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded border", getTagClasses(tag.color))}>
                {tag.name}
              </span>
            ))}
          </div>
          ...
          <div className="mb-3 pl-2">
            <h4 className="text-sm font-bold text-gray-800 leading-tight mb-1">{item.content}</h4>
          ...
          <div className="flex items-center justify-between border-t border-gray-50 pt-2 pl-2">
            ...
            {item.assignee && (
               <div className="w-5 h-5 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 border border-white shadow-sm" title={item.assignee} />
            )}
          </div>
        </motion.div>
      </div>
...
const KanbanColumn = ({ id, column, activeId, onDrop, setHoverState }) => {
  ...
  return (
    <div 
      className="flex flex-col min-w-[340px] w-[340px] mx-2 h-fit transition-all"
      onDragOver={(e) => {
        e.preventDefault();
    ...
    >
      <div className="px-3 py-3 flex items-center justify-between bg-[#FDFDFD] rounded-t-xl border border-[#FDFDFD]/50 shadow-sm  sticky top-0 z-10">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-extrabold text-gray-700 uppercase tracking-wide">{column.title}</h2>
          <span className="bg-gray-100 text-gray-500 text-[10px] font-bold px-2 py-0.5 rounded-full border border-gray-200">
            {column.items.length}
          </span>
        </div>
        <div className="flex gap-1">
          <CirclePlus size={16} className="text-gray-400 cursor-pointer hover:text-blue-600 transition-colors" />
        </div>
      </div>

      <div className="flex-1 px-1 min-h-[150px] pt-2 pb-10 custom-scrollbar bg-[#F9F9F9] rounded-b-xl">
        ...
        {column.items.length === 0 && (
           <div className="h-24 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center text-gray-400 text-xs bg-gray-50/50">
             Drop items here
           </div>
        )}
      </div>
...
export default function Kanban() {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F3F4F6] font-sans text-slate-900 overflow-hidden">
      <header className="px-6 py-4 bg-white border-b border-gray-200 flex-none z-20 shadow-sm flex justify-between items-center">
        <div className="flex items-center gap-2">
          <div className="p-2 bg-blue-600 rounded-lg">
            <LayoutGrid size={18} className="text-white" />
          </div>
          <span className="font-bold text-lg tracking-tight text-gray-800">Kanban Board</span>
        </div>
      </header>
      ...
      <div 
        ...
        className="flex-1 overflow-auto bg-[#F3F4F6] relative"
      >
        <div className="flex px-6 py-6 w-max h-full items-start gap-2">
          ...
          <div className="min-w-[340px] px-2 opacity-50 hover:opacity-100 transition-opacity">
            <button className="flex items-center justify-center gap-2 text-gray-500 hover:text-blue-600 transition-colors w-full h-[50px] border-2 border-dashed border-gray-300 rounded-xl hover:border-blue-400 hover:bg-blue-50">
              <Plus size={20} />
