...
import { Calendar, MessageSquare, LayoutGrid, Plus, MoreHorizontal, MessageCircle, CirclePlus } from 'lucide-react';
    ...
    <Draggable draggableId={item.id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          <div
            className={cn(
              "bg-white p-3 rounded-xl transition-all duration-200 group relative border border-gray-200",
              snapshot.isDragging 
                ? "shadow-2xl ring-2 ring-blue-500/50 rotate-2 scale-105 z-50" 
...
const KanbanColumn = ({ columnId, column }) => {
  return (
    ...
    <div className="flex flex-col min-w-[342px] w-[342px] mx-2 bg-[#FDFDFD] rounded-xl border border-[#FDFDFD]/50 h-fit">
      ...
      <Droppable droppableId={columnId}>
        {(provided, snapshot) => (
          <div
            ...
            ref={provided.innerRef}
            className={cn(
              'flex-1 px-3 pb-4 transition-colors duration-200 min-h-[150px]', // Min-height ensures drop area exists
